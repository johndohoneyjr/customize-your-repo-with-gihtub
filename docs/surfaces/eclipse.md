...
| License | Commercial — Free (Copilot subscription required); plugin source moving to MIT |
...
The single most common failure mode on the Eclipse plugin is the Copilot Language Server failing to reach GitHub's APIs from behind a corporate TLS-intercepting proxy. Symptoms include:
...
Export the proxy's root CA (Zscaler, Netskope, Palo Alto, etc.) as a PEM file and point `NODE_EXTRA_CA_CERTS` at it. Importing the certificate into the system trust store is not sufficient — Node.js does not read the Windows Trust Store or the macOS Keychain by default.
