...
- **JS/TS Chat Features skill bundle.** A built-in skill bundle for JavaScript and TypeScript workspaces, toggleable with `jsts-chat-features.skills.enabled`. It provides language-aware behaviors such as test scaffolding and import fixing without a plugin install.
- **Foreground terminal tool support.** Agent-invoked terminal commands can run in the foreground terminal the user is looking at, so long-running tasks can be monitored and interrupted with Ctrl+C.
- **Tool confirmation carousel.** When an agent queues multiple tool calls, confirmations are grouped into a carousel so you can approve, deny, or edit them as a batch.
- **Background terminal notifications on by default.** When a backgrounded terminal command finishes or needs input, the editor surfaces a notification.
